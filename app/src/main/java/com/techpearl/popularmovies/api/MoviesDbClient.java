...
public interface MoviesDbClient {
    ...
    static final String API_VEDIO_PATH = "videos";
    ...
    @GET(API_MOVIE_PATH + "/{id}" + API_VEDIO_PATH)
    Call<List<Video>> movieTrailers(@Path("id") String movieId, @Query(API_KEY_PARAM) String apiKey);

    @GET(API_MOVIE_PATH + "/{id}" + API_REVIEWS_PATH)
    Call<List<Review>> movieReviews(@Path("id") String movieId, @Query(API_KEY_PARAM) String apiKey);
}
